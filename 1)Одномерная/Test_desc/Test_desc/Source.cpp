...
//градиент исследуемой функции
{
	...
	tmp.push_back(4 * x[0]*x[0]*x[0] - 4 * x[0]*x[1] + 2 * x[0] - 2);
	tmp.push_back(2 * (x[1] - x[0] * x[0]));
...
//minimizes N-dimensional function f; x0 - start point
{
	...
	for (Iterations = 1; Iterations <= NUMBER_OF_ITERATIONS; Iterations++)
	{
		...
		if (METHOD == 3)
		{
			cur_x = goldensectionoptimize(cur_x, -10, 1110, 100);
		}
...
int main()
{
	...
	x.push_back(3);
	int i, Iteration;
	vector<double> ans = GradientDescent(3, x, Iteration);
